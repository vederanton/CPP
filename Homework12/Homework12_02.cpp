...
int main() {
	...
	std::cout << "\nТретий массив с общими элементами из обоих массивов:" << std::endl;
	coutAndClearArr3(arr3);

	addItems3(arr1, SIZE, arr2, SIZE, arr3);
	std::cout << "\nТретий массив с элементами, которые есть только в первом массиве:" << std::endl;
	coutAndClearArr3(arr3);

	addItems3(arr1, SIZE, arr2, SIZE, arr3);
	addItems3(arr2, SIZE, arr1, SIZE, arr3); // Для четвертого пункта задания просто повторим эту функцию, поменяв местами массивы
	std::cout << "\nТретий массив с элементами, которые есть только в одном из массивов:" << std::endl;
	coutAndClearArr3(arr3);
