...
double Polygon::lengthSide(Coordinate one, Coordinate two) const {
	...
	xDiff = fabs(two.x_ - one.x_); // определ€ем катет по ’ с положительным знаком
	yDiff = fabs(two.y_ - one.y_); // определ€ем катет по ” положительным знаком
	return lengthSide = sqrt(pow(xDiff, 2) + pow(yDiff, 2));
}

double Polygon::calculateSides() const { // сумма длин всех сторон
	double sumSides = 0;
	for (auto it = polygon_.begin(); it != polygon_.end() - 1; ++it) { // сумма сторон дл€ всех сторон кроме последней
		sumSides += lengthSide(*it, *(it + 1));
	}
	return sumSides += lengthSide(polygon_[0], polygon_[polygon_.size() - 1]); // прибавл€ем последнюю
}
